...
### getInstalledAppBundleList
### Returns the list of app bundles installed and its corresponding version
###
### Arguments:
### No arguments
###
...
getInstalledAppBundleList()
{
        ...
        if [ -d "$BUNDLE_METADATA_NVM_APPS_PATH" ]; then
                log "Fetching app bundle metadata from $BUNDLE_METADATA_NVM_APPS_PATH"
                metadata_nvm_ls="$(find $BUNDLE_METADATA_NVM_APPS_PATH -name "*_package.json" -type f | tr '\n' ' ' | xargs)"
        else
                log "$BUNDLE_METADATA_NVM_APPS_PATH does not exist"
        fi

        if [ -d "$BUNDLE_METADATA_RFS_APPS_PATH" ]; then
                log "Fetching app bundle metadata from $BUNDLE_METADATA_RFS_APPS_PATH"
                metadata_rfs_ls="$(find $BUNDLE_METADATA_RFS_APPS_PATH -name "*_package.json" -type f | tr '\n' ' ' | xargs)"
        else
                log "$BUNDLE_METADATA_RFS_APPS_PATH does not exist"
        fi

        if [ -z "$metadata_rfs_ls" -a -z "$metadata_nvm_ls" ]; then
                log "No app bundle metadata found in CPE"
                metadata_ls=""
        elif [ -z "$metadata_rfs_ls" ]; then
                log "App bundle metadata found only in $BUNDLE_METADATA_NVM_APPS_PATH"
                metadata_ls="$metadata_nvm_ls"
        elif [ -z "$metadata_nvm_ls" ]; then
                log "App bundle metadata found only in $BUNDLE_METADATA_RFS_APPS_PATH"
                metadata_ls="$metadata_rfs_ls"
        else
                log "App bundle metadata found in both $BUNDLE_METADATA_NVM_APPS_PATH & $BUNDLE_METADATA_RFS_APPS_PATH"
                log "Preparing final app metadata list"
                metadata_ls="$metadata_nvm_ls"
                for metadata in $metadata_rfs_ls; do
                        echo "$metadata_nvm_ls" | grep -q "$(basename $metadata)"
                        if [ $? -eq 0 ]; then
        ...
        list=""
        if [ -n "$metadata_ls" ]; then
                log "List of app bundle metadata found in CPE: $metadata_ls"
                for file in $metadata_ls; do
                        bundle_name=$(getPkgMetadata $file $PKG_METADATA_NAME)
                        bundle_version=$(getPkgMetadata $file $PKG_METADATA_VER)
        ...
        if [ -n "$list" ]; then
                # Remove leading and trailing spaces if any
                bundle_list=$(echo $list | xargs | tr ' ' ',')
                log "Installed app bundle list: $bundle_list"
        else
                log "Installed app bundle list empty"
                bundle_list=""
        fi
